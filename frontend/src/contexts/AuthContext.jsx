...
export const AuthProvider = ({ children }) => {
  ...
  const register = async (name, email, password, birthDate, gender) => {
    try {
      ...
      const response = await axios.post(
        `${process.env.REACT_APP_API_URL || 'http://hamza-aourass.ddns.net:5001'}/api/auth/register`,
        {
          firstName,
          lastName,
  ...
  const registerProfessional = async (
    ...
    try {
      ...
      const response = await axios.post(
        `${process.env.REACT_APP_API_URL || 'http://hamza-aourass.ddns.net:5001'}/api/auth/register/professional`,
        {
          firstName,
          lastName,
          name: `${firstName} ${lastName}`,
  ...
  const loginWithCredentials = async (email, password) => {
    try {
      setLoading(true);
      const response = await axios.post(
        `${process.env.REACT_APP_API_URL || 'http://hamza-aourass.ddns.net:5001'}/api/auth/login`,
        { email, password }
      );
  ...
  const sendPasswordResetEmail = async email => {
    try {
      setLoading(true);
      const response = await axios.post(
        `${process.env.REACT_APP_API_URL || 'http://hamza-aourass.ddns.net:5001'}/api/auth/forgot-password`,
        { email }
      );
  ...
  const resetPassword = async (token, password) => {
    try {
      setLoading(true);
      const response = await axios.post(
        `${process.env.REACT_APP_API_URL || 'http://hamza-aourass.ddns.net:5001'}/api/auth/reset-password/${token}`,
        { password }
      );
