...
const AdminProductsPage = () => {
  ...
  const fetchProducts = async (page = 1, search = '', category = '', status = '') => {
    try {
      ...
      const response = await axios.get(
        `${process.env.REACT_APP_API_URL || 'http://hamza-aourass.ddns.net:5001'}/api/admin/products`,
        {
  ...
  const toggleApproval = async (productId, currentStatus) => {
    try {
      ...
      await axios.patch(
        `${process.env.REACT_APP_API_URL || 'http://hamza-aourass.ddns.net:5001'}/api/admin/products/${productId}/approve`,
        { status: newStatus },
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const updateProduct = async e => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.put(
        `${process.env.REACT_APP_API_URL || 'http://hamza-aourass.ddns.net:5001'}/api/admin/products/${selectedProduct._id}`,
        formData,
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const deleteProduct = async productId => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.delete(
        `${process.env.REACT_APP_API_URL || 'http://hamza-aourass.ddns.net:5001'}/api/admin/products/${productId}`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
