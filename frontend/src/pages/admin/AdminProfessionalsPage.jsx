...
const AdminProfessionalsPage = () => {
  ...
  const fetchProfessionals = async () => {
    try {
      ...
      const response = await axios.get(
        `${process.env.REACT_APP_API_URL || 'http://hamza-aourass.ddns.net:5001'}/api/admin/professionals?${queryParams}`,
        {
  ...
  const handleCreateProfessional = async data => {
    try {
      ...
      const response = await axios.post(
        `${process.env.REACT_APP_API_URL || 'http://hamza-aourass.ddns.net:5001'}/api/admin/professionals`,
        data,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
  ...
  const handleDeleteProfessional = async id => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.delete(
        `${process.env.REACT_APP_API_URL || 'http://hamza-aourass.ddns.net:5001'}/api/admin/professionals/${id}`,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );
