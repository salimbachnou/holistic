...
const AdminBookingsPage = () => {
  ...
  const fetchBookings = async () => {
    try {
      ...
      const response = await axios.get(
        `${process.env.REACT_APP_API_URL || 'http://hamza-aourass.ddns.net:5001'}/api/admin/bookings?${queryParams}`,
        {
          headers: {
  ...
  const updateBookingStatus = async (bookingId, newStatus, adminNotes) => {
    try {
      const token = localStorage.getItem('token');
      await axios.put(
        `${process.env.REACT_APP_API_URL || 'http://hamza-aourass.ddns.net:5001'}/api/admin/bookings/${bookingId}/status`,
