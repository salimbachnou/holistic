...
const AdminClientsPage = () => {
  ...
  const fetchClients = async (page = 1, search = '') => {
    try {
      ...
      const response = await axios.get(
        `${process.env.REACT_APP_API_URL || 'http://hamza-aourass.ddns.net:5001'}/api/admin/clients`,
        {
  ...
  const toggleVerification = async (clientId, currentStatus) => {
    try {
      const token = localStorage.getItem('token');
      await axios.patch(
        `${process.env.REACT_APP_API_URL || 'http://hamza-aourass.ddns.net:5001'}/api/admin/clients/${clientId}/verify`,
  ...
  const deleteClient = async clientId => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.delete(
        `${process.env.REACT_APP_API_URL || 'http://hamza-aourass.ddns.net:5001'}/api/admin/clients/${clientId}`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
